...
def liked_meals():
    ...
    # Fetch liked meals for the user from recent_activity
    liked_meals_data = supabase.table("recent_Activity").select("*").eq("User_Id", user_id).eq("Liked", 1).execute().data
    liked_meal_ids = [meal['Meal_Id'] for meal in liked_meals_data]

    # Fetch meal details from the dataset table
    if liked_meal_ids:
        liked_meals_query = supabase.table("dataset").select("*").in_("Meal_Id", liked_meal_ids).execute()
        liked_meals = liked_meals_query.data
    else:
        liked_meals = []

    return render_template('liked.html', liked_meals=liked_meals)
...
def feedback():
    ...
    if request.method == 'POST':
        ...
        meal_feedback = request.form.get('Meal_Feedback', '').strip()
        model_feedback = request.form.get('Model_Feedback', '').strip()
        general_feedback = request.form.get('General_Feedback', '').strip()
        print(meal_feedback)
        ...
        try:
            ...
            supabase.table("feedback").insert(feedback_entry).execute()
    ...
    # Render feedback form with pre-filled user details
...
def process():
    ...
    if language == "English":
        ...
        1. Display as points or short para nutritional values in descending order, covering calories, protein, fat, and carbohydrates.
        2. illustrate the nutritional contribution of each ingredient, unraveling the dietary secrets within in small points or short paras.
        """
        input_prompt4 = """
